...
async def _get_vllm_response_with_dup_retry(
    *,
    ...
    meta: Dict[str, Any] = {"dup_reprompts": 0, "used_summary": False, "history_summary": ""}

    def _messages(system_msg: Dict[str, Any] | None, hist: List[Dict[str, Any]], text: Any) -> List[Dict[str, Any]]:
        return sanitize_messages(system_msg=system_msg, history=hist, user_text=text)

    system_prompt = base_system_prompt
    messages_for_llm = _messages(system_prompt, history, user_text)
    ...
    for i in range(max_dup_reprompts):
        ...
        messages_for_llm = _messages(retry_system_prompt, history, user_text)
        response_text = await vllm_client.get_vllm_response(
            messages_for_llm,
    ...
    if summary:
        ...
        messages_for_llm = _messages(summarized_system_prompt, [], user_text)
        response_text = await vllm_client.get_vllm_response(
            messages_for_llm,
